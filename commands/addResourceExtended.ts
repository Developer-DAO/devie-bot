...
export async function execute(interaction: CommandInteraction) {
  ...
  if (!source) {
    interaction.reply({ content: 'Source required, please try submitting again.', ephemeral: true });
    return;
  }

  if (!isValidUrl(source)) {
    interaction.reply({ content: 'Invalid Source provided, please check it before submitting again.', ephemeral: true });
    return;
  }

  const foundResouce = await findResourceByUrl(source);
  if (foundResouce) {
